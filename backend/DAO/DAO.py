...
def security_check_objects(user_id: str, objects: typing.List):
    assert isinstance(user_id, str) and len(user_id) > 10

    for obj in objects:
        if obj["user_id"] != user_id: 
            raise Exception("Unexpected data loaded from database! Shut down the system and analyse the incident")
            return
    return        
...
class DAO:
    ...
    def __set_user_id(self, cur: psycopg.Cursor, user_id: str):
        cur.execute(sql.SQL('SET "mimr.current_user" = {};').format(sql.Literal(user_id)))
      
    @contextmanager
    def rls_cursor(self, user_id: str):
        conn = self.__get_connection()
        try:
            cur = conn.cursor()
            self.__set_user_id(cur, user_id)
            yield cur
            conn.commit()
        except Exception:
            conn.rollback()
    ...
    @dao_exception_handler
    def get_all_projects(self, user_id: str) -> typing.List[ProjectApi]: 
        with self.rls_cursor(user_id) as cur:
            cur.execute("SELECT user_id, id, name FROM projects WHERE user_id = %s ORDER BY name", (user_id, ))
            res = cur.fetchall()

        security_check_objects(user_id, res)
        return [ProjectApi(**obj) for obj in res]
